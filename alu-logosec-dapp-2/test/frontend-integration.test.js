...
describe("Frontend Integration Tests", async function () {
  const { viem } = await network.connect();
  const [ownerWallet, addr1Wallet] = await viem.getWalletClients();
  ...
  before(async function () {
    ...
    assetRegistry = await viem.deployContract("ALUAssetRegistry");
    logoToken = await viem.deployContract("ALULogoToken", [ownerWallet.account.address]);
    ...
    // Token ID 0 is now registered with ALU_LOGO_HASH
  });
  ...
  it("Test 9: frontend reads total ALUT supply as 1,000,000", async function () {
    const totalSupply = await logoToken.read.totalSupply();
    const decimals = 10n ** 18n;
    const formatted = Number(totalSupply / decimals);
    // The frontend displays this value; assert it equals 1,000,000
    expect(formatted).to.equal(1_000_000);
  });
  ...
  it("Test 11: verifyLogoIntegrity shows verification SUCCESS for correct hash", async function () {
    ...
    const tokenId = 0n;
    const [isVerified] = await assetRegistry.read.verifyLogoIntegrity([tokenId, ALU_LOGO_HASH]);
    ...
    const asset = await assetRegistry.read.getAsset([tokenId]);
    expect(asset.name).to.equal("ALU Official Logo 2026");
  });
  ...
  it("Test 12: verifyLogoIntegrity shows verification FAILURE for wrong hash", async function () {
    ...
    const tokenId = 0n;
    const [isVerified] = await assetRegistry.read.verifyLogoIntegrity([tokenId, WRONG_HASH]);
  ...
  it("Test 13: distributeShares updates recipient balance after successful transfer", async function () {
    const wholeTokens = 50_000n;              // contract expects whole tokens
    const decimals = 10n ** 18n;
    const expectedDelta = wholeTokens * decimals;

    // Check initial balance
    const balanceBefore = await logoToken.read.balanceOf([addr1Wallet.account.address]);

    // Owner distributes shares to addr1 (mirrors the dashboard distribute form)
    await logoToken.write.distributeShares([addr1Wallet.account.address, wholeTokens]);

    // Check updated balance
    const balanceAfter = await logoToken.read.balanceOf([addr1Wallet.account.address]);

    expect(balanceAfter - balanceBefore).to.equal(expectedDelta);

    // Verify ownership percentage updated (50,000 / 1,000,000 = 5%)
    const pct = await logoToken.read.ownershipPercentage([addr1Wallet.account.address]);
    // Check it's non-zero
    expect(pct).to.be.gt(0n);
  });
});
